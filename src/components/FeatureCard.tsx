...
const statusStyles: Record<Feature['status'], { label: string; className: string }> = {
  ...
  'coming-soon': {
    label: 'Coming Soon',
    className: 'bg-surface-500/15 text-surface-400 border-surface-500/30',
  },
}
...
export default function FeatureCard({ feature, index }: FeatureCardProps) {
  ...
  return (
    <motion.div
      ...
      className={`card-glow group relative rounded-2xl border border-white/[0.06] bg-white/[0.03] p-5 backdrop-blur-sm transition-colors ${
        isDisabled
          ? 'cursor-not-allowed opacity-50'
          : 'cursor-pointer hover:border-white/[0.12] hover:bg-white/[0.06]'
      }`}
    >
      {/* Icon with glow */}
      ...
      {/* Content */}
      <h3 className="mb-1 text-[15px] font-semibold text-white/90 group-hover:text-white">
        {title}
      </h3>
